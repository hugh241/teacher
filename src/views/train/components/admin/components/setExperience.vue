...
<template>
  <div class="setExperience">
    ...
    <ExperDetail v-if="show" :show="show" @close="handelClose" @save="handelSave" :width="addwidth" :editData="editData"
      :title="title"></ExperDetail>
...
// import ExperDetail from '@/views/train/components/admin/components/detail/experDetail.vue'
import utils from '@/utils/downLoad.js'
export default {
  components: {
    Tabs: () => import('./tabs/index.vue'),
    Find: () => import('./find/index.vue'),
    ExperDetail: () => import('@/views/train/components/admin/components/detail/experDetail.vue'),
  ...
  methods: {
    ...
    downClick(val) {
      ...
      utils.downloadEvent('post', '培训心得列表', '/studyNotes/exportNotesWorkList', false, data)
    ...
    async getStateNum() {
      const res = await getStateNum({ type: '4', courseId: this.courseId })
      if (res.returnCode == 200) {
    ...
    async findNotesList(val) {
      const res = await findNotesList({ state: '4', userId: val, pageNo: 1, pageSize: 10 })
      if (res.returnCode == 200) {
        this.show = true
