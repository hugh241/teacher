  ...
  <div class="ques-feedback">
    <el-card class="box-card" v-if="editinfo">
      <el-form
        ...
        label-width="120px"
      >
        <el-form-item label="心得要求：">
          <span style="color: red">{{
            courseDetail.experienceNeed
              ? courseDetail.experienceNeed
              : "暂无要求"
          }}</span>
        </el-form-item>
    ...
    <el-card class="box-card" v-if="!editinfo">
      ...
      >
        ...
        <el-form-item label="附件：">
          ...
          <!-- <el-upload class="upload-demo" ref="upload" name="fileData" :data="submitData" :action="uploadUrl"
            :headers="upHeaders" :auto-upload="false" :show-file-list="false" :on-progress="handleProgress"
            :on-change="handleChange" :on-remove="handleRemove" :on-success="handleSuccess" :on-error="handleError"
            :limit="3" :accept="accept" :on-exceed="handleExceed">
          </el-upload> -->
          <el-table :data="tableData" border style="width: 100%" size="mini">
            <el-table-column label="文件名称" prop="fileName">
            </el-table-column>
...
<script>
import { addStudyNotes, findNotesList } from "@/api/learningCenter";
    ...
    onSubmit() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          console.log(this.fileIds, this.ruleForm.fileIds);
          addStudyNotes(this.ruleForm).then((res) => {
            if (res.returnCode == 200) {
              this.editinfo = false;
              this.$emit("refreshFun");
              this.$store.dispatch("type/saveTableData", 1);
    ...
    getList() {
      findNotesList({
        ...
        if (res.returnCode == "200" && res.returnData) {
          ...
          this.myArticleContent = res.returnData.content;
